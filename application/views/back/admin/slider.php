...
<div id="content-container">
	<div id="page-title">
		<h1 class="page-header text-overflow"><?php echo translate('Управлять ползунком слоя');?></h1>
	</div>
	<div class="tab-base">
		<div class="panel">
                ...
                <div class="tab-content">
                    <div class="col-md-12" style="border-bottom: 1px solid #ebebeb;padding: 5px;">
                        <button class="btn btn-primary btn-labeled fa fa-plus-circle add_pro_btn pull-right" 
                        	onclick="ajax_set_full('add','<?php echo translate('Загрузить'); ?>',
                            '<?php echo translate('Успешно добавлен!'); ?>','slider_add','')">
								<?php echo translate('Создать слайдер');?>
                        </button>
                        <button class="btn btn-info btn-labeled fa fa-plus-circle add_pro_btn pull-right" 
                        	onclick="ajax_set_list()">
								<?php echo translate('Список слайдеров');?>
                        </button>
                        <button class="btn btn-purple btn-labeled fa fa-align-justify add_pro_btn pull-right" 
                        	onclick="ajax_set_full('serial','<?php echo translate('Серийный слайдер'); ?>',
                            '<?php echo translate('Успешно сериализован!'); ?>','slider_serial',''); ">
								<?php echo translate('Серийный слайдер');?>
                        </button>
                        <div class="col-sm-6">
                            <input id="set_slider" class='sw' data-set='set_slider' 
                            type="checkbox" <?php if($this->crud_model->get_type_name_by_id('general_settings','53','value') == 
                            'ok'){ ?>checked<?php } ?> />
                        </div>
                    </div>
                    <!-- LIST -->
...
<script>
    ...
    $(document).ready(function(){
        $(".sw").each(function(){
            ...
            var id = h.attr('id');
            var set = h.data('set');
            new Switchery(document.getElementById(id), {color:'rgb(100, 189, 99)', secondaryColor: '#cc2424', 
