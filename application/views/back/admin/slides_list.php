		...
		data-ignorecol="0,4" data-show-toggle="true" data-show-columns="false" data-search="true" >
			...
			?>
                <tr>
                    ...
                    <td>
                    	<input id="slide_<?php echo $row['slides_id']; ?>" class="slide" 
						type="checkbox" data-id="<?php echo $row['slides_id']; ?>" 
						<?php if($row['status']=='ok'){ echo 'checked'; } ?> />
                    </td>
                    <td class="text-right">
                        <a class="btn btn-success btn-xs btn-labeled fa fa-wrench" data-toggle="tooltip" 
                            onclick="ajax_modal('edit','<?php echo translate('Редактировать слайды'); ?>',
							'<?php echo translate('Успешно отредактирован!'); ?>','slides_edit',
							'<?php echo $row['slides_id']; ?>')" 
	...
	<div id='export-div'>
		...
		<table id="export-table" data-name='slides' data-orientation='p' style="display:none;">
				...
				<tr>
					...
					<td><?php echo $this->crud_model->
					get_type_name_by_id('category',$row['category'],'category_name'); ?></td>
				</tr>
	            <?php
	            	}
				?>
...
function set_switchery(){
	$(".slide").each(function(){
		new Switchery(document.getElementById('slide_'+$(this).data('id')), {color:'rgb(100, 189, 99)', 
		secondaryColor: '#cc2424', jackSecondaryColor: '#c8ff77'});
